from brownie import AGU, config, network, accounts
...
def deploy():
    ...
    deploy_wallet = AGU.deploy({"from": account}, publish_source = True)
    print(f"Contract deployed successfully at {deploy_wallet.address}")

    with open("../Deployment Addresses.txt", "r+") as dep:
        dep.write("AGU => https://rinkeby.etherscan.io/address/"+deploy_wallet.address+"\n\n")
